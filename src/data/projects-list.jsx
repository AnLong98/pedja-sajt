import arima from "../../public/arima.webp"
import tf from "../../public/teams_filtering.webp"
import snake from "../../public/snake.webp"
import ftp from "../../public/ftp.webp"
import cv from "../../public/cv.webp"
import smart from "../../public/smart.webp"
import solidity from "../../public/solidity.webp"
import trainicon from "../../public/trainicon.webp"
import payment from "../../public/payment.webp"

export const projectsList = [
    {
        name: "Crypto ARIMA",
        github: "https://github.com/AnLong98/SIAP_2022",
        text: "Cryptocurrency price prediction using ARIMA and ARIMAX models.",
        image: arima
    },

    {
        name: "Teams filtering",
        github: "https://github.com/AnLong98/Teams-filtering",
        text: "Marathon race results handling made in Java Swing. ",
        image: tf
    },

    {
        name: "Snake Game",
        github: "https://github.com/AnLong98/DRS2020_presteje",
        text: "Multiplayer snake game that can be played on LAN, coded in Python.",
        image: snake
    },

    {
        name: "Torrent clone",
        github: "https://github.com/AnLong98/IKP_2020",
        text: "Peer to peer file transfer proof-of-concept implementation in C++.",
        image: ftp
    },


    {
        name: "CV Elastic",
        github: "https://github.com/AnLong98/UDD",
        text: "Job application handling with Elasticsearch.",
        image: cv
    },

    {
        name: "Smart Energy",
        github: "https://github.com/AnLong98/Predmet-projekat",
        text: "Microservice application for power system outage management.",
        image: smart
    },
    {
        name: "Solid lottery",
        github: "https://github.com/AnLong98/solid-lottery",
        text: "Decentralized smart contract lottery implementation in Solidity.",
        image: solidity
    },

    {
        name: "RVA Railway",
        github: "https://github.com/AnLong98/RVA-2020-Railway_System",
        text: "Railway management system implementation in .NET WPF.",
        image: trainicon
    },

    {
        name: "Payment center",
        github: "https://github.com/mas-projekti/SEP_Projekat",
        text: "Payment processing API integrated with Paypal and Bitcoin.",
        image: payment
    }

];